...
from mmaudio.model.low_level import MLP, ChannelLastConv1d, ConvMLP, Attn_Net_Gated
...
class MMAudio(nn.Module):
    ...
    def __init__(self,
        ...
        self.text_input_proj = nn.Sequential(
            ...
            MLP(hidden_dim, hidden_dim, hidden_dim * 4),
        )
        ...
        self.global_cond_mlp = MLP(hidden_dim * 2, hidden_dim, hidden_dim * 4)
        # each synchformer output segment has 8 feature frames
        # self.audio_pool = Attn_Net_Gated(hidden_dim)
    ...
    def update_seq_lengths(self, latent_seq_len: int, clip_seq_len: int, sync_seq_len: int, audio_seq_len: int) -> None:
        self._latent_seq_len = latent_seq_len
        self._clip_seq_len = clip_seq_len
        self._sync_seq_len = sync_seq_len
        self._audio_seq_len = audio_seq_len
    ...
    def predict_flow(self, latent: torch.Tensor, t: torch.Tensor,
        ...
        global_c = self.global_cond_mlp(torch.cat([semantic_c, audio_f_c], dim=-1))  # (B, D)
